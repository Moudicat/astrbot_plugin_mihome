...
DEVICE_PROFILES = {
    "空调插座": {
        "prop_map": {
            "温度": "target_temperature", "风速": "fan_level", "模式": "mode",
            "上下扫风": "vertical_swing", "扫风": "vertical_swing"
        },
        "value_map": {
            "制冷": "cool", "制热": "heat", "自动": "auto",
            "开": True, "关": False, "开扫风": True, "关扫风": False
        },
        "display_map": {
            "ac_work_mode": "空调工作模式", "vertical_swing": "上下扫风状态",
    ...
    "空气净化器": {
        "prop_map": {
            "模式": "mode", "风速": "fan_level", "童锁": "physical_controls_locked",
            "提示音": "alarm", "屏幕": "brightness"
        },
        "value_map": {
            "自动": 0, "睡眠": 1, "最爱": 2, "亮": 0, "暗": 1, "熄灭": 2
        },
    ...
    "空气炸锅": {
        ...
        "display_map": {
            "target_time": "设定时间", "target_temperature": "设定温度",
            "recipe_name": "当前食谱", "turn_pot": "翻锅提醒状态",
            "current_keep_warm": "当前正处保温", "reservation_left_time": "距离预约开始",
            "texture": "口感设置"
        },
        "detail_writable": ["on", "mode", "target_time", "target_temperature", "auto_keep_warm", "turn_pot_config", "texture", "cooking_weight"],
    ...
    "蒸煮锅": {
        ...
        "display_map": {
            "target_time": "设定时间", "target_temperature": "设定温度",
            "temperature": "锅内实时温度", "cook_done": "烹饪是否完成",
            "recipename": "当前食谱", "warm_temperature": "设定保温温度",
            "switch_pausetoadd": "中途加料开关"
        },
        "detail_writable": ["on", "mode", "target_time", "target_temperature", "heat_level", "keepwarm_set", "alarm", "cookreservation", "switch_pausetoadd"],
    ...
    "落地扇": {
        ...
        "value_map": {
            "直吹": "straight", "自然风": "nature", "开": True, "关": False
        },
...
def get_device_prop_map(official_name: str) -> Dict[str, str]:
    for key, profile in DEVICE_PROFILES.items():
        if key in official_name: return {**GLOBAL_PROP_MAP, **profile.get("prop_map", {})}
    return GLOBAL_PROP_MAP

def get_device_val_map(official_name: str) -> Dict[str, Any]:
    for key, profile in DEVICE_PROFILES.items():
        if key in official_name: return {**GLOBAL_VAL_MAP, **profile.get("value_map", {})}
    return GLOBAL_VAL_MAP

def get_device_display_map(official_name: str) -> Dict[str, str]:
    for key, profile in DEVICE_PROFILES.items():
        if key in official_name: return {**GLOBAL_DISPLAY_MAP, **profile.get("display_map", {})}
...
def get_device_detail_writable_keys(official_name: str) -> List[str]:
    for key, profile in DEVICE_PROFILES.items():
        if key in official_name: return profile.get("detail_writable", [])
    return ["on"]

def get_device_detail_readable_keys(official_name: str) -> List[str]:
    for key, profile in DEVICE_PROFILES.items():
        if key in official_name: return profile.get("detail_readable", [])
    return []

def get_device_help_examples(official_name: str) -> Dict[str, List[str]]:
    for key, profile in DEVICE_PROFILES.items():
        if key in official_name: return profile.get("help_examples", {})
    return {}

def get_device_help_hints(official_name: str) -> Dict[str, str]:
    for key, profile in DEVICE_PROFILES.items():
        if key in official_name: return profile.get("help_hints", {})
    return {}
